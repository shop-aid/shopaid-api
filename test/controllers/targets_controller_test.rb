...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post targets_url, params: { target: { cause_id: @target.cause_id, user_id: @target.user_id } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { cause_id: @target.cause_id, user_id: @target.user_id } }
